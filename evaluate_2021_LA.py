...
def performance(cm_scores, Pfa_asv, Pmiss_asv, Pfa_spoof_asv, invert=False):
    ...
    return min_tDCF, eer_cm, threshold
...
def eval_to_score_file(score_file, cm_key_file):
    ...
    min_tDCF, eer_cm, eer_threshold = performance(cm_scores, Pfa_asv, Pmiss_asv, Pfa_spoof_asv)

    out_data = "min_tDCF: %.4f\n" % min_tDCF
    out_data += "eer: %.2f\n" % (100*eer_cm)
    out_data += "Threshold at EER: %.4f\n" % eer_threshold
    print(out_data, end="")

    # just in case that the submitted file reverses the sign of positive and negative scores
    min_tDCF2, eer_cm2, eer_threshold2 = performance(cm_scores, Pfa_asv, Pmiss_asv, Pfa_spoof_asv, invert=True)
