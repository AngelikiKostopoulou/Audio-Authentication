#!/usr/bin/env python
"""
Script to compute EER and plot ROC curves for multiple models (wav2vec, whisper, MFCC) on the DF dataset.
Usage:
$: python evaluate_all2021DF_roc.py PATH_TO_GROUNDTRUTH_DIR phase

-PATH_TO_GROUNDTRUTH_DIR: path to the directory that has the CM protocol
-phase: either progress, eval, or hidden_track

Example:
$: python evaluate_all2021DF_roc.py ./keys eval
"""
...
def plot_multiple_roc_curves(models_data, output_filename="roc_curve_allmodels21DF.png"):
    """
    ...
    plt.title('ROC Curve for all Models in ASVspoof2021 DF')
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python evaluate_all2021DF_roc.py PATH_TO_GROUNDTRUTH_DIR phase")
        exit(1)
    
    truth_dir = sys.argv[1]
    phase = sys.argv[2]
    
    cm_key_file = os.path.join(truth_dir, 'DF/trial_metadata.txt')  # Update path for DF metadata
    
    # Paths to score files for each model
    wav2vec_score_file = "eval_CM_190633_wav2vec_DF_new.txt"  
    whisper_score_file = "eval_CM_DF_whisper_1505.txt"
    mfcc_score_file = "eval_CM_DF_mfcc_2505.txt"
