...
class SSLModel(nn.Module):
    def __init__(self, device):
        ...
        self.model.encoder.conv1.register_forward_hook(make_inputs_require_grad)
    ...
    def extract_feat(self, input_data):
        ...
        if isinstance(input_data, str):
            signal, sample_rate = sf.read(input_data)
            if len(signal.shape) > 1:
                signal = signal.mean(axis=1)
            input_data = torch.tensor(signal, device=self.device).unsqueeze(0)
        
        # Process with Whisper
        features = self.processor(input_data.cpu().numpy(), return_tensors="pt", sampling_rate=16000)
        input_features = features["input_features"].to(self.device)
        with torch.no_grad():
            emb = self.model.encoder(input_features).last_hidden_state
