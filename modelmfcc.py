...
class MFCCModel(nn.Module):
    def __init__(self, device):
        ...
        self.mfcc_transform = torchaudio.transforms.MFCC(
            ...
            melkwargs={
                ...
                'n_mels': 80,  # Reduced from 128 to 80
                'f_min': 20,
                'f_max': 8000,
            }
        )
        ...
        # Project MFCC features to the required dimension
        self.projection = nn.Sequential(
            nn.Linear(self.n_mfcc, 256),
            nn.ReLU(),
    ...
    def extract_feat(self, input_data):
        ...
        #if self.mfcc_transform.melkwargs['n_fft'] != input_data.device:
        #    self.mfcc_transform = self.mfcc_transform.to(input_data.device)
        ...
        # Extract MFCCs
        mfcc_features = self.mfcc_transform(input_tmp)  # [batch, n_mfcc, time]
        
        # Transpose to [batch, time, n_mfcc]
        mfcc_features = mfcc_features.transpose(1, 2)
        
        # Project to required dimension
        emb = self.projection(mfcc_features)  # [batch, time, out_dim]
...
class Model(nn.Module):
    def __init__(self, args,device):
        ...
        ####
        self.ssl_model = MFCCModel(self.device)
