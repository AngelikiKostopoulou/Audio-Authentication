#!/usr/bin/env python
"""
Script to compute EER and plot ROC curves for multiple models (wav2vec, whisper, MFCC) on the Glow TTS dataset.
Usage:
$: python evaluate_all_Glow_roc.py PATH_TO_SCORE_FILES

Example:
$: python evaluate_all_Glow_roc.py ./
"""
...
def plot_multiple_roc_curves(models_data, output_filename="roc_curve_allmodels_Glow.png"):
    """
    ...
    plt.title('ROC Curve for all Models on Glow TTS Dataset')
...
def evaluate_model_roc(score_file, cm_key_file, model_name):
    """
    Evaluate a single model and return ROC data using Glow TTS format.
    """
    print(f"Evaluating {model_name}...")
    print(f"Score file: {score_file}")
    print(f"Metadata file: {cm_key_file}")
    
    # Process metadata file specially - it has a unique format (same as evaluate_glow)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python evaluate_all_Glow_roc.py PATH_TO_SCORE_FILES")
        exit(1)
    
    score_files_dir = sys.argv[1]
    
    # Glow TTS metadata file path (same as in evaluate_glow)
    ...
    # Paths to score files for each model (update these to match your Glow TTS score files)
    wav2vec_score_file = os.path.join(score_files_dir, "eval_CM_Glow_wav2vec_13061.txt")  
    whisper_score_file = os.path.join(score_files_dir, "eval_CM_Glow_whisper_13061.txt")
    mfcc_score_file = os.path.join(score_files_dir, "eval_CM_Glow_mfcc_13061.txt")
