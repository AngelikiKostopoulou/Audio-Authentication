#!/usr/bin/env python
"""
Script to compute EER and plot ROC curves for multiple models (wav2vec, whisper, MFCC) on the In-the-Wild dataset.
Usage:
$: python evaluate_all_InTheWild_roc.py PATH_TO_SCORE_FILES

Example:
$: python evaluate_all_InTheWild_roc.py ./
"""
...
def plot_multiple_roc_curves(models_data, output_filename="roc_curve_allmodels_InTheWild.png"):
    """
    ...
    plt.title('ROC Curve for all Models on In-the-Wild Dataset')
...
def evaluate_model_roc(score_file, cm_key_file, model_name):
    """
    Evaluate a single model and return ROC data using In-the-Wild format.
    """
    print(f"Evaluating {model_name}...")
    print(f"Score file: {score_file}")
    print(f"Metadata file: {cm_key_file}")
    
    # Process metadata file specially - it has a unique format (same as evaluate_in_the_wild)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python evaluate_all_InTheWild_roc.py PATH_TO_SCORE_FILES")
        exit(1)
    
    score_files_dir = sys.argv[1]
    
    # In-the-Wild metadata file path (same as in evaluate_in_the_wild)
    ...
    # Paths to score files for each model (update these to match your In-the-Wild score files)
    wav2vec_score_file = os.path.join(score_files_dir, "eval_CM_InTheWild_wav2vec_09_05.txt")  
    whisper_score_file = os.path.join(score_files_dir, "eval_CM_InTheWild_whisper1505.txt")
    mfcc_score_file = os.path.join(score_files_dir, "eval_CM_inthewild_mfcc_2505.txt")
